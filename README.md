...
| GET | `/api/allure/testresult/{id}/screenshot` | Скриншот падения из вложений Allure |
...
`https://dersecur.testops.cloud/launch/{launchId}/tree/{testResultId}`:

- **Stacktrace из Allure** — message и trace через API;
- **Скриншот падения** — изображение из вложений test result (обычно `*-screenshot*.png`).
...
| `CACHEBUST` | Опционально: смените значение (например `2`), если повторный деплой того же коммита не обновил код — Timeweb мог взять Docker-слои из кэша |
...
`REPORT_AUTH_INITIAL_ACCESS_KEY` читается напрямую из env (по аналогии с `ALLURE_TESTOPS_TOKEN`) и используется как стартовый ключ при запуске приложения.  
После генерации нового ключа в админке активным становится новый ключ, а стартовый перестаёт подходить.
