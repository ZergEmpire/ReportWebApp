...
Эндпоинты `/sendMessage` и `/pinChatMessage` **не требуют** ключа доступа (исключены из авторизации). Просмотр Dashboard — по ключу, если включён `report.auth.enabled`.
...
По умолчанию (`report.auth.enabled=true`) Dashboard и `/api/*` требуют ключ доступа.  
Админ: страница входа → блок администратора → **Ключ доступа** / **Категории**.

Переменные: `REPORT_AUTH_ENABLED`, `REPORT_AUTH_ADMIN_USERNAME`, `REPORT_AUTH_ADMIN_PASSWORD`.
